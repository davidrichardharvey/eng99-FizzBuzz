...
while prompt_if_game_over:
    ...
    while prompt_if_number:
        Number_goes_upto = input("Enter the number up to which you would like to play the FIZZBUZZ Game\n")
        if Number_goes_upto.isdigit():
            prompt_if_number = False
        else:
            print("Error: Kindly  entered a valid number (eg 100)")


    for number in range(1,int(Number_goes_upto)+1):
